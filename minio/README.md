...
```
```bash
helm upgrade --install s3 "bitnami/minio" -f values.yml -n testing --version "14.1.6"
```
...
#### Get manifests:
```bash
helm template s3 "bitnami/minio" -f values.yml -n testing --version "14.1.6" > manifests.yml
```
