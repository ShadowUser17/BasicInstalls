...
```
```bash
helm upgrade --install s3 "bitnami/minio" -f values.yml -n testing --version "14.1.1"
```
...
#### Get manifests:
```bash
helm template s3 "bitnami/minio" -f values.yml -n testing --version "14.1.1" > manifests.yml
```
