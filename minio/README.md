...
```
```bash
helm upgrade --install s3 "bitnami/minio" -f values.yml -n testing --version "14.1.4"
```
...
#### Get manifests:
```bash
helm template s3 "bitnami/minio" -f values.yml -n testing --version "14.1.4" > manifests.yml
```
